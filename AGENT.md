...
> Single source of truth for AI dev. Read fully before code/architecture/decisions.

---

## Project Overview

**FinSolve** — internal enterprise chatbot. Employees query company docs via natural language. RBAC enforced — each employee sees only authorized data. Guardrails block PII leakage + off-topic queries. Fully local via Docker Compose. Monitoring + eval pipelines wired.

---

## Absolute Constraints — Never Violate

1. **Role ALWAYS extracted server-side from verified JWT** — never from request body
2. **Qdrant payload filters ALWAYS applied before returning chunks** — never unfiltered chunks to LLM
3. **PII regex scanner ALWAYS on LLM output** before frontend send
4. **JWT in httpOnly cookie only** — never localStorage/sessionStorage
5. **Nemotron = cloud inference** — avoid raw PII (names, salaries, SSNs) in prompts. Flag if encountered.
6. **All retrieval anchoring prompts: LLM never speculates beyond retrieved context**
...
**Role values (JWT + Qdrant):** `engineering` | `finance` | `marketing` | `hr` | `c_suite` | `employee`

---

## Metadata Schema — Every Chunk Must Carry
...
### Markdown Documents
- **Primary split:** `###` headers (Level 3)
- **Target:** 300–500 tokens/chunk
- **Long subsections:** split at `####`
- **Short sections** (<150 tokens): merge with sibling `####`
- **Tables:** one atomic chunk — never split mid-table
...
- **Parent-child:** child (`####`, ~150 tokens) for retrieval; parent (`###`, ~400 tokens) for LLM context

### Excel HR Data
- **Row-level chunking:** one chunk per employee record
- **Extra metadata:** `employee_id`
- **Never** mix rows into single chunk
...
- **Dark theme:** slate-900 bg, slate-800 sidebar, slate-700 assistant bubbles, blue-600 user bubbles
- **Streaming:** `ReadableStream` + `getReader()` — never buffer full response before render
- **Guardrail block:** show `GuardrailBlockedMessage` with specific `block_reason` — not generic error
- **Source citations:** always render `SourceCitations` below assistant messages
- **AdminPanel:** render only if `role === 'c_suite'`
